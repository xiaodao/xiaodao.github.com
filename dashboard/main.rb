	...
	def read_build_status
		open("dashboard.html", "w") { |file|
			...
			builds.each{ |build|
				...
				codes.each{ |code|
					file << "<li>" + code.inner_html.gsub("/MyAccountUI/branches/R5/", "").gsub("mock-service-federator-client/","").gsub("presentation/","").gsub('src/main/webapp/','').gsub('src/test/java/', "").gsub('src/main/java/', "").gsub("src/main/resources/", "").gsub("telstra", "t***a").gsub("myaccount", "m***t") + "</li>"
				}
				file << "</ul></td></tr>"
				end
			}
